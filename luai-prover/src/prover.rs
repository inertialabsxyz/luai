...
use luai::{
    ...
    zkvm::{
        commitment::{compute_public_inputs, PublicInputs},
...
impl<H: HostInterface> Prover<H> {
    ...
    pub fn dry_run(
        ...
        Ok(DryRunResult { output, oracle_tape, public_inputs })
    }
    ...
    // ── Proof generation (requires RISC Zero) ─────────────────────────────────
    //
    // The `prove` method would call the RISC Zero prover here:
    //
    // pub fn prove(&self, guest_input: GuestInput) -> Receipt {
    //     let env = ExecutorEnv::builder()
    //         .write(&guest_input).unwrap()
    //         .build().unwrap();
    //     let prover = default_prover();
    //     prover.prove(env, LUAI_GUEST_ELF).unwrap()
    // }
}
